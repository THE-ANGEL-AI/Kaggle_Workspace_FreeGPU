...
### Git
- Ветка: `main`
- Репо: `github.com/THE-ANGEL-AI/Kaggle_Workspace.git`
- Последний коммит: `5b3361f` — refactor: скрипты в instal/, обновлён .gitignore и README

### Структура репозитория на GitHub
```
Kaggle_Workspace/              # корень репо
├─ .gitignore                  # whitelist: только README.md + instal/*.py
├─ README.md                   # публичный README (бейджи, инструкции)
└─ instal/
   ...
   ├─ .gitignore                  # скрывает docs/, .env*, __pycache__/
   └─ docs/                       # локально, НЕ на GitHub
      ├─ AGENTS.md                # Инструкция для агентов
      └─ MEMORY.md                # Этот файл
```

**Важно:** В репозитории на GitHub публикуется ТОЛЬКО 7 файлов (см. выше).
Всё остальное (блокноты, воркфлои JSON, токены, angel-ui, Claude Code Docs,
_kaggle_tests, PROJECT_MEMORY.md) — только локально, защищено .gitignore.
...
### 2026-06-19 — Чистка remote Kaggle_Workspace, структура instal/
**Сделано:**
- Сброшена локальная main к remote (remote — канон: kaggle_env.py, LTX 2.3, WhatDreamsCost)
- Скрипты перенесены из корня в папку `instal/` (git mv, структура Kaggle_Workspace/instal/*.py)
- Удалён встроенный `.git` в `instal/` (был клон того же репо — вызывал submodule-предупреждение)
- Корневой `.gitignore`: whitelist-стратегия, защита `.env*`, токенов, блокнотов, воркфлоев JSON, сторонних папок
- `instal/.gitignore`: скрывает `docs/`, `.env*`, `__pycache__/` (локальные файлы агентов и токены)
- Корневой `README.md`: профессиональный вид, бейджи (Boosty, ВК, GPU, Python, torch), актуальные 8 нод + 12 моделей (Flux2 + LTX 2.3), блок «Быстрый старт» с правильным clone URL `Kaggle_Workspace`
- Удалён дубликат `instal/README.md`
- Запушен коммит `5b3361f` от THE-ANGEL-AI на `origin/main`
- Очищены unreachable loose objects (git gc --prune=now)

**Файлы затронуты:** .gitignore, README.md, instal/.gitignore, instal/instal_comfyui.py, instal/instal_castom_node.py, instal/kaggle_env.py, instal/start.py
